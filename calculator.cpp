...
bool isOperator(char op) { return op == '+' || op == '-' || op == '*' || op == '/' || op == '%'; }

double evaluate(std::vector<std::string> inputTokens, std::vector<int> inputTokenTypes) {
    std::vector<std::string> tokens;
    std::vector<int>
        tokenTypes; // 0 for number, 1 for standard operator, 2 for single number operator
    ...
    while (!completed) {
        ...
        while (i < tokens.size()) {
            ...
            case 1:
                if (tokens[i] == "//") {
                    newTokens[i - 1] =
                        std::to_string((int)(std::stod(tokens[i - 1]) / std::stod(tokens[i + 1])));
                ...
                case '+':
                    newTokens[i - 1] =
                        std::to_string(std::stod(tokens[i - 1]) + std::stod(tokens[i + 1]));
                    newTokens.erase(newTokens.begin() + i, newTokens.begin() + i + 2);
                    newTokenTypes.erase(newTokenTypes.begin() + i, newTokenTypes.begin() + i + 2);
                    newTokenPriority.erase(newTokenPriority.begin() + i,
                                           newTokenPriority.begin() + i + 2);
                ...
                case '%':
                    newTokens[i - 1] = std::to_string((int)std::stod(tokens[i - 1]) %
                                                      (int)std::stod(tokens[i + 1]));
                    newTokens.erase(newTokens.begin() + i, newTokens.begin() + i + 2);
                    newTokenTypes.erase(newTokenTypes.begin() + i, newTokenTypes.begin() + i + 2);
                    newTokenPriority.erase(newTokenPriority.begin() + i,
                                           newTokenPriority.begin() + i + 2);
